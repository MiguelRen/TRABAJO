...
$query3= "select * from control_lotes where num_lote=$field4";
$conn->query($query3);
...
if ($field1=='ENTRADA'){
	...
	$query2 = "INSERT INTO ajustes (tipo, cod_barra_art, descripcion_art, cantidad, lote, fecha_venc, fecha_aju, realizado)
	 ...
	 ($field1=='SALIDA' and $field3>=$datos3 and $_SESSION['name']=='supervisor' ){
		$query4 = "UPDATE art SET EXISTENCIA=existencia-$field3 WHERE COD_BARRA=$field2";
		$query5 = "UPDATE control_lotes SET exis_lote=exis_lote-$field3 WHERE num_lote=$field4";
		$query2 = "INSERT INTO ajustes (tipo, cod_barra_art, descripcion_art, cantidad, lote, fecha_venc, fecha_aju, realizado)
            VALUES ('{$field1}','{$field2}','$datos1','{$field3}','{$field4}','{$fechav}',now(),'{$_SESSION['name']}')";
          ...
          echo 'LLAME A UN SUPERVISOR PARA HACER ESTE AJUSTE';
		  }
...
else echo"CREE EL PRODUCTO ANTES DE AJUSTARLO. PRODUCTO NO EXISTE";
...
</td><td> <h1 id="logo"><center><a >AJUSTE DE INVENTARIO  </a></center></h1> </td> <td align='right'> 	 <?= $_SESSION['name'] ?> <a href="cerrar-sesion.php" style="color:white;"><i class="fas fa-sign-out-alt"></i>Cerrar Sesión</a>	</td>
...
// Create connection
$conn = mysqli_connect($servername, $username, $password, $database);
$query="SELECT ID,tipo FROM tipo_ajuste ORDER BY tipo ASC; ";
