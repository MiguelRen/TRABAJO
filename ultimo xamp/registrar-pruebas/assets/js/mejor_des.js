...
const fetchDataMejorDespachador = async (form_data) => {
    try {
        const data = await app('http://192.168.0.164/registrar-pruebas/controllers/mejor_despachador.php?extract_statistic=1','POST',form_data);
        data_estadisticas = data;
      } catch (error) {
        console.log(error);
      }
};
...
//Capturtamos el evento submit y ejecutamos logica para realizar peticiones a backend
d.addEventListener("submit", e => {
    ...
    fetchDataMejorDespachador(formData);
