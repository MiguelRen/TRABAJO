...
$query = "INSERT INTO fallas (NUM_PEDIDO, NOMB_EMPLEADO,NOMB_RECHEQUEADOR, MOTIVO, DESCRIPCION, FECHA)
            VALUES ('{$field1}','{$field3}','{$_SESSION['name']}','{$field2}','{$field4}',now())";

$query2 ="UPDATE pedidos SET NOMB_RECHEQUEADOR ='{$_SESSION['name']}' where NUM_PEDIDO='$field1'";
$conn->query($query);
$conn->query($query2);
mysqli_close($conn);
?>
...
</td><td> <h1 id="logo"><center><a >Registrar Falla </a></center></h1> </td> <td align='right'> 	 <?= $_SESSION['name'] ?> <a href="cerrar-sesion.php" style="color:white;"><i class="fas fa-sign-out-alt"></i>Cerrar Sesión</a>	</td>
...
<Table>						
   <tr> 
  <td class="td0" > FALLA REGISTRADA </td>
  </tr>
</Table>					
<?php
$servername = "192.168.0.164";
...
// Create connection
$conn = mysqli_connect($servername, $username, $password, $database);
$query="SELECT ID,MOTIVO FROM MOTIVOS ORDER BY MOTIVO ASC; ";
...
<Table>						
   ...
   <td class="td0" > DESCRIPCION: <input type="text" name = "field4" /> </td></tr>
</Table>	
